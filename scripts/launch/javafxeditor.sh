...
find_jar() {
    ...
    # 回退到项目构建版本
    for version in "1.0.1" "1.0.0-SNAPSHOT"; do
        if [ -f "$project_dir/JavaFxEditor/target/$app_name-$version.jar" ]; then
            echo "$project_dir/JavaFxEditor/target/$app_name-$version.jar"
            return 0
        fi
        # 尝试 runnable JAR
        if [ -f "$project_dir/JavaFxEditor/target/$app_name-$version-runnable.jar" ]; then
            echo "$project_dir/JavaFxEditor/target/$app_name-$version-runnable.jar"
            return 0
        fi
    done
...
APP_JAR=$(find_jar "JavaFxEditor")

if [ -z "$APP_JAR" ] || [ ! -f "$APP_JAR" ]; then
    echo "❌ 错误: 未找到 JavaFxEditor JAR 文件"
    echo ""
    echo "请选择以下方式之一："
